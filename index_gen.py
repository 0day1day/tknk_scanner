...
def find_signatures(root):
        ...
        if root=="yara/rules/malware":
            duplicated = official_duplicated
        elif root=="yara/CAPE":
            duplicated = cape_duplicated

        p = pathlib.Path(root)
        file_list = list(p.glob('**/*'))
        for p_entry in file_list:
            if p_entry.suffix == ".yara" or p_entry.suffix == ".yar":
                if (p_entry.name not in ignore) and (p_entry.name not in duplicated) and (p_entry.name not in no_use):
                    entry = str(p_entry)
...
if __name__ == "__main__":
        generated = []
        paths = ["yara/my_rules","yara/Neo23x0", "yara/rules/malware", "yara/CAPE", "yara/malpedia_yara"]
