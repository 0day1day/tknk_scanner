...
no_use = ["MALW_CAP_Win32Inet.yara", "MALW_LURK0.yar", "MALW_Surtr.yar", "RAT_Bolonyokte.yar", "MALW_Glasses.yar", "MALW_IcedID.yar"]

def find_signatures(root):
        signatures = []
        for entry in os.listdir(root):
            if (entry.endswith(".yara") or entry.endswith(".yar")) and (entry not in ignore) and (entry not in duplicated) and (entry not in no_use):
